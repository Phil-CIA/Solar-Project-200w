...
Critical correction 2026-09-03: the rework described above as an R33 zero-ohm replacement was actually performed on R28. The manufactured Rev 0 netlist confirms R28 is the 51.7 kOhm U6 pin 12 `RT` resistor, R29 is the 20.5 kOhm U6 pin 8 `CFG2` resistor and remained unchanged, and R33 is the U6 pin 9 `ADDR_SLOPE_CFG1` resistor. R33 remained fitted at 6.49 kOhm throughout the NACK tests, which selects Table 7-4 row 8 and disables I2C. Therefore all original-U6 and replacement-U6 NACK results were expected for the actual fitted configuration, and the resulting device/package/reset/ground diagnoses are withdrawn.

R33 was then actually changed to zero ohms. At the bounded powered point, measurements relative to `PWR_NEG` were `nRST=3.29 V`, `AGND=0.0 V`, `PGND=0.0 V`, accessible exposed-pad copper `=0.0 V`, `VCC2=4.96 V`, `VCC1=5.06 V`, and `EN_UVLO=1.43 V`. Source current was below the bench supply display resolution and no more than approximately 1.3 mA. One CR-terminated address-only `u6 probe` command on verified `COM5` returned `U6 0x6A ACK`. No register address was transmitted and `CLEAR_FAULTS` was not accessed. This provisionally closes the LM51772 communication blocker; `FLT` remains unresolved.

Next gate: with every source removed and all rails below 0.1 V, restore R28 from zero ohms to its intended 51.7 kOhm `RT` value. Leave R29 at its intended 20.5 kOhm value, keep R33 at zero ohms, and retain the PGND-to-PWR_NEG jumper. Verify those four paths unpowered. Then repeat one address-only probe after a complete cold power cycle at the existing bounded source point. If it ACKs, use only the existing read-only `u6 status` command for `STATUS_BYTE` register `0x78`; do not write registers or access `CLEAR_FAULTS` at `0x03`.

R28 restoration and first valid status 2026-09-03: R28 was restored to 51.7 kOhm, R29 was verified at 20.5 kOhm, and R33 was verified at zero ohms. With PV at the existing bounded point, COM5 returned `STATUS OCP=1 OVP=0 PA8_PWM=0 PB5_EN=0 PA5_LEGACY=0`. An initial address-only probe produced no captured UART text; a following non-I2C status query proved UART remained responsive. One controlled repeat with a longer receive window returned `U6 0x6A ACK`.

The existing read-only `u6 status` command then returned `U6 STATUS_BYTE=0x58 BUSY=0 OFF=1 VOUT=0 IOUT=1 INPUT=1 TEMP=0 CML=0 OTHER=0`. Per TI, `OFF` indicates output below power-good or converter disabled, `IOUT` indicates latched inductor peak-current-limit detection, and `INPUT` indicates latched VDET or UVLO below-threshold detection. No register was written and `CLEAR_FAULTS` register `0x03` was not accessed. LM51772 communication is no longer blocked. Remove PV power before further diagnosis. Do not clear faults or authorize switching; next determine whether `IOUT` repeats after a clean power cycle with restored R28 and M1-M4 still absent.

IOUT repeat check 2026-09-03: PV was fully power-cycled with R28 restored. MCU safety state again passed with `PA8_PWM=0 PB5_EN=0 PA5_LEGACY=0`. A second read-only `u6 status` returned the identical `STATUS_BYTE=0x58`, so `IOUT` relatches in the corrected configuration and is not stale from the zero-ohm RT state. TI provides no more detailed latched IOUT status register; `USB_PD_STATUS_0` at `0x21` reports only instantaneous constant-current operation. Remove PV and inspect the R26/R27/C20 `CSA`/`CSB` network unpowered before any static differential measurement. Do not clear faults or attempt switching.

IOUT isolation 2026-09-04: the R26/R27/C20 network passed unpowered continuity, resistance, and short checks. With PV at 6.49 V / 20 mA, `CSA=3.2041 V`, `CSB=3.2043 V`, and the direct differential across C20 was approximately `0.1 mV`. These balanced DC values disprove a sustained peak-current condition. After full discharge, a 5.0 V cold start drew less than 1 mA and produced `EN_UVLO=1.0241 V`, below the rising threshold. MCU safety status was `OCP=1 OVP=0 PA8_PWM=0 PB5_EN=0 PA5_LEGACY=0`.

COM5 did not accept the subsequent text command. SWD was therefore bound to verified ST-LINK `B55B5A1A0000000020AFF501`; target voltage was 3.246 V and Device ID was `0x469`. With the MCU briefly halted to avoid bus contention, PB6/PB7 configuration saved, and all power-control GPIOs untouched, one read-only software-I2C transaction read U6 register `0x78` as `STATUS_BYTE=0x48`: `OFF=1 IOUT=0 INPUT=1`. The script restored PB6/PB7 and resumed the MCU. It did not write flash, option bytes, or any LM51772 register and did not access `CLEAR_FAULTS` at `0x03`. Therefore the 6.49 V `IOUT` latch occurs only after crossing UVLO into an attempted startup with M1-M4 absent; it is not present during inhibited 5.0 V power-up and is not a sustained DC sense fault. No switching is authorized.

COM5 recovery 2026-09-04: Windows still identified the exact onboard CP210x as `USB\VID_10C4&PID_EA60\0001` on COM5 with status OK. A bounded 115200 8N1 attempt returned `ERR unknown command`, proving both UART directions worked and indicating that earlier uncompleted text remained in the firmware's partial command buffer. The parser clears its buffer after processing an unknown line. One subsequent CR-terminated `status` command returned `STATUS OCP=1 OVP=0 PA8_PWM=0 PB5_EN=0 PA5_LEGACY=0`. COM5 is healthy; no UART rework or alternate COM port is required.

M1-M4 installation 2026-09-04: ten available devices were identified as Infineon `BSC030N04NSGATMA1`, whose `PG-TDSON-8-1` package and source/gate/exposed-drain mapping match the Rev 0 land pattern. With all sources removed and rails below 0.1 V, M2, M3, M1, and M4 were installed sequentially. Each device passed visual orientation/joint inspection, gate-to-source and gate-to-drain short checks, expected body-diode polarity, reverse blocking after gate discharge, and adjacent-rail resistance checks. Final checks found no input-to-return, output-to-return, or `SW_1`-to-`SW_2` near-short. U2/U5 bypasses and the PGND-to-PWR_NEG jumper remained unchanged. Photographs covering both board sides and all four joints were captured. The installation-only gate is complete. No power application or switching is authorized under the installation plan.
