...
TI LM51772-Q1 Rev. A Section 7.3.7.2 explicitly states, "Do not connect an external load to the VCC2-PIN." D8 has been isolated from `U6_VCC2`, measuring 300 kOhm from D8 VDD to U6 pin 29. The 6.5 V / 20 mA source point was applied at J1 `PV_IN_POS_RAW`; correction recorded 2026-09-01 establishes that U6 `PV_IN_POS` was approximately 4.54 V, not 6.49 V. `EN_UVLO=1.030 V` is therefore broadly consistent with the fitted divider and below its rising threshold. H3 remains blocked by the approximately 1.95 V front-end loss from `PV_IN_POS_RAW` to `PV_IN_POS`. Do not raise the source or command `V_in_ON`/PA5 with PV present until the fitted U2 pinout and U2/Q2/R40-R43/D1 control topology are verified. The next powered point must record `PV_IN_POS_RAW`, `PV_IN_POS_FUSED`, and `PV_IN_POS` as distinct nodes.

U2 diagnostic closure 2026-09-01: `PV_IN_POS_RAW=6.492 V`, `PV_IN_POS_FUSED=6.492 V`, `PV_IN_POS=4.543 V`, U2 gate = 6.42 V, D1 anode-to-cathode = 1.92 V, and source current = 416 uA. The approximately +1.92 V U2 gate-to-common-source bias confirms threshold-region conduction caused by the invalid gate-drive topology. H3 shall not continue through U2. With all sources removed and rails discharged, select either a documented temporary `PV_IN_POS_FUSED`-to-`PV_IN_POS` bypass for isolated low-energy bench testing or permanent front-end rework. A temporary bypass defeats reverse blocking and prohibits battery, panel, load, switching, and unattended operation.

Temporary U2 and U5 bypasses were installed 2026-09-01. With TP4 / `PV_IN_POS` equal to `PV_IN_POS_RAW=6.49 V`, U6 `EN_UVLO=1.43 V`, `VCC1=5.0375 V`, `VCC2=4.9706 V`, `FLT=0.02 V`, and `RST=3.29 V`. Inline current was 2.1 mA versus the supply's coarse approximately 1 mA indication. This is the first point above the U6 rising UVLO threshold; do not increase voltage. H3 remains partial pending acceptable thermal behavior and confirmation that HO1, LO1, HO2, and LO2 are static with M1-M4 unpopulated. The U2/U5 bypasses defeat reverse blocking and remain restricted to isolated, attended, current-limited bench operation with no battery, output source, load, or switching.

Bounded H3 static result: pass for thermal stability and inactive gate outputs at 6.49 V. U6 and both bypass jumpers remained cool with no odor, instability, or current limiting. M2 G-S / LO1, M3 G-S / LO2, M1 G-S / HO1-SW1, and M4 G-S / HO2-SW2 each measured 0 V by DMM at the unpopulated footprints. H3 remains partial overall because `FLT=0.02 V` is still asserted and the input/output protection paths are bypassed. No voltage increase, switching, MOSFET population, battery, output source/load, panel, or unattended operation is authorized.
