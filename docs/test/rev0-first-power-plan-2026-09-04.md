...
- **FP3** - above-UVLO PV static with U6 held disabled (switching inhibited by EN_UVLO hold-low).
- **FP4** - first controlled converter start (switching). **Blocked**: defined here only as an entry gate and hazard bound; it requires its own reviewed authorization and may not be run in the same session as FP0-FP3.
...
- **Accepted for FP0-FP3 only**, under these compensating controls: isolated bench source, hard source current limit (the only overcurrent protection with F1 bypassed), attended operation, and stop thresholds below.
...
- **FP4 precondition**: before FP4, a reviewer must either (a) restore/rework F1, U2, and U5, or (b) record a written waiver stating why bench-only switching with bypasses is acceptable, with the source current limit and preload named as the substitute protection. Option (b) is acceptable only for the first bounded switching characterization; any battery/panel/inverter work requires (a).
...
- Static stages (FP1-FP3): DMM measurements suffice; a ground-referenced scope probe is permitted only on `PWR_NEG`-referenced nets (`CTRL_3V3`, `VBUS`, `EN_UVLO`, `LO1`, `LO2`, `PV_IN_POS`).
...
At each powered stage, log before/after (and at 60 s intervals during FP3/FP4): M1-M4 cases, L2, R22 shunt, U6, the U2/U5 bypass jumpers, and the F1 jumper — by IR thermometer or calibrated touch. Stop thresholds: any component more than 15 °C above ambient in FP1-FP3 (nothing should be dissipating), or more than 40 °C above ambient in FP4, or any odor, discoloration, or audible instability.
...
| FP3 | Isolated bench PV at **6.5 V / 20 mA**, with `EN_UVLO` held low (Section 12) | Open | Input current > 10 mA sustained, any gate ≥ 0.1 V, any SW-node movement, HB-SW > 6 V |
| FP4 | Isolated bench PV at **6.5 V / 200 mA** initial (review may adjust before sign-off) | Resistive preload sized at review (target ~10-25% of expected minimum converter load; no battery, no backfeed-capable device) | SW overshoot > 32 V, off-gate > 1.5 V, bootstrap collapse, current limit, thermal limits, COM5/SWD dropout, run time > 30 s |
...
## 12. FP3 EN_UVLO Hold-Low Provision

FP3 exists to prove the populated board's static rails above UVLO without permitting switching. Method: a documented, photographed temporary jumper pulling the `EN_UVLO` node to `PGND` **installed and verified unpowered** before the source is raised past 5.5 V, and removed only after the source is off and rails are discharged. If review rejects grounding `EN_UVLO` (e.g., divider loading concern), FP3 is skipped and its checks fold into a more conservative FP4 entry. Rationale: with M1-M4 installed, "above-UVLO static" does not otherwise exist — U6 will attempt startup.

Note: the historical `IOUT` latch (`STATUS_BYTE=0x58`) was associated with UVLO crossing while the bridge was absent. FP3/FP4 must re-read `STATUS_BYTE` and record it; a latched fault with the populated bridge is new information, not a repeat, and is a stop-and-review event. It is not a justification to write `CLEAR_FAULTS`.
...
|---|---|---|
| FP0 | Plan reviewed and signed | No - review pending |
| FP1 | FP0 pass recorded | No - review pending |
| FP2 | FP1 pass recorded | No - review pending |
| FP3 | FP2 pass + EN_UVLO hold-low method approved | No - review pending |
| FP4 | FP3 pass (or documented skip) + bypass disposition (Section 3) + differential probe available + preload value signed | **No - requires separate authorization session** |

## 16. Rev 1 Corrections Reaffirmed (not blocking FP0-FP3)
