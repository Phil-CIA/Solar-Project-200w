...
|---|---|---|---|---|
| Q1 | High-side buck switch | N-channel MOSFET | Vds >= 40V, Id >= 30A, low Rds(on) class | Place close to D3 and L1 around switch node |
| D3 | Freewheel path (asynchronous) | Schottky diode | Vr >= 60V, If(avg) >= 10A, surge-capable package | Place tight to Q1 source and return path |
...
| C4 | Input decoupling #1 | Ceramic capacitor | 4.7uF to 10uF, 63V, X7R | Place very close to Q1 drain-source loop |
| C5 | Input decoupling #2 | Ceramic capacitor | 1uF to 4.7uF, 63V, X7R | Place beside C4 for HF support |
...
| C8 | Snubber capacitor (placeholder) | Ceramic capacitor | 470pF to 2.2nF, 63V NP0/X7R | In series with R1, close to switch loop |
...
- TI calculator guidance for the synchronous FET sizing pass points to roughly 30 A and 25.6 V Vds for this envelope, so the existing 40 V MOSFET set is acceptable here.
- U2 overvoltage shutdown is the primary high-input protection mechanism, so the FET selection does not need to chase a 60 V-class ceiling for this stage.
