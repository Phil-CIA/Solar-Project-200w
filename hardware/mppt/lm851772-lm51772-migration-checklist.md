...
Separate finding (2026-09-01, unrelated to I2C/ADDR strap): `U6` pin 7 (`MODE`) and pin 16 (`ILIMCOMP/ISET`) share one net, `Net-(U6-ILIMCOMP_ISET)`, alongside `R32`. `MODE` must be a clean digital logic input per the datasheet ("Do not leave this pin floating") and must not share a node with the analog `ISET`/compensation pin. This is a distinct wiring defect, confirmed via bench meter as a real physical trace short; verify with a meter whether U6 pin 7 is truly shorted to pin 16 on the physical board, and separate them in the schematic regardless. See `SOLAR_HANDOFF_2026-09-01.md` for rework steps.

**Open issue tracked for next revision (2026-09-01):** once the `MODE`/`ILIMCOMP_ISET` trace is cut on the bench, `MODE` is left with only `R32` (100 kOhm to `PWR_NEG`/AGND) — electrically this is a valid, clean logic low (`VIL` max 0.4 V per datasheet Table 5.3), so it is not unsafe, but it **hard-configures Power Save Mode (PSM) permanently enabled** (MODE low → PSM; MODE high → forced PWM/CCM, per Section 7.3.2). No GitHub Issue could be filed automatically for this in the current environment (no issue-creation tool available), so it is logged here and in `SOLAR_HANDOFF_2026-09-01.md` for a maintainer to transcribe into an actual tracked Issue. Rev 2 action: decide PSM-always vs. forced-PWM as the intended production default, and if forced-PWM is wanted, give `MODE` its own dedicated strap net separate from `R32`/`ISET` rather than relying on the incidental logic-low it inherits today.
