...
Correction (2026-09-01): the `ILIMCOMP/ISET` net (`R32`, pin 16) was found netlist-shorted to `MODE` (pin 16 and pin 7 share `Net-(U6-ILIMCOMP_ISET)` in `hardware/kicad/solar-project/report.txt`). `MODE` is a datasheet-required digital logic input ("Do not leave this pin floating") and must not share a node with the analog `ISET`/compensation pin. This must be separated (schematic fix + bench verification of any physical short) before the current-limit threshold can be trusted; see `SOLAR_HANDOFF_2026-09-01.md` for detail.
