...
| I2C/control | SDA/SCL, address/config, reset/fault pins aligned | FAIL | Root cause found 2026-09-01 for the "no I2C ack from U6" bring-up finding. Per LM51772-Q1 datasheet Section 7.5.1/Table 7-4, I2C on U6 pin 9 (`ADDR/SLOPE`, net `ADDR_SLOPE_CFG1`) is enabled **only** when that pin is a direct short to GND (address `0x6A`) or to VCC2 (address `0x6B`). Any resistor from that pin instead selects a slope-compensation factor and explicitly **disables** the I2C interface, repurposing `SDA/CFG3` and `SCL/CFG4` (U6 pins 5/6, routed to the shared software-I2C bus alongside AHT20 `0x38` and SSD1306 `0x3C`) as CFG3/CFG4 strap pins. This project's schematic/netlist (`report.txt` line 1248) shows `R33` = 6.49 kOhm from `ADDR_SLOPE_CFG1` to `PGND`, which lands on the 6.5 kOhm row of Table 7-4 (`msc = 1`, I2C DISABLED) instead of a direct GND short. This fully explains why `i2cscan`/H5 only found `0x38`/`0x3C` and never `0x6A`/`0x6B`, independent of VCC1/VCC2/EN_UVLO health. Rework option: remove/bypass R33 and short U6 pin 9 directly to PGND (selects address `0x6A`) or to VCC2 (selects `0x6B`); do not leave any resistor in that path if I2C control is required. Rev 1 should replace R33 with a 0 Ohm link (or direct trace) to GND or VCC2 per the chosen address, and update the current-limit slope compensation design (previously reliant on the R33 resistor value) to use the I2C `ILIM_THRESHOLD`/DAC path instead. |
